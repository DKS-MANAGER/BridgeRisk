...
def evaluate_model(model, X, y, threshold=0.5, model_name=""):
    ...
    metrics = {
        ...
        "pr_auc": (
            average_precision_score(y, y_proba) if len(np.unique(y)) > 1 else np.nan
        ),
        ...
        "false_negative_rate": (
            ...
            if (confusion_matrix(y, y_pred)[1, 0] + confusion_matrix(y, y_pred)[1, 1])
            > 0
            else 0
        ),
    }
...
def main():
    ...
    pd.DataFrame(cm_data).to_csv(
        os.path.join(REPORTS_DIR, "confusion_matrices.csv"), index=False
    )
    ...
    for name in MODEL_NAMES:
        ...
        ax.plot(
            fpr,
            tpr,
            label=f"{name} (AUC={all_metrics[MODEL_NAMES.index(name)]['roc_auc']:.3f})",
        )
    ...
    for name in MODEL_NAMES:
        precision, recall, _ = precision_recall_curve(
            y_test, predictions[name]["proba"]
        )
        ax.plot(
            recall,
            precision,
            label=f"{name} (PR-AUC={all_metrics[MODEL_NAMES.index(name)]['pr_auc']:.3f})",
        )
