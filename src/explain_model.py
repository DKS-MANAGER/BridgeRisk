...
def main():
    ...
    print(
        "High-risk bridge ID: "
        f"{test_df.loc[high_risk_idx, 'bridge_id']} "
        f"(prob={test_df.loc[high_risk_idx, 'predicted_proba']:.3f})"
    )
    print(
        "Low-risk bridge ID: "
        f"{test_df.loc[low_risk_idx, 'bridge_id']} "
        f"(prob={test_df.loc[low_risk_idx, 'predicted_proba']:.3f})"
    )
    ...
    plt.title(
        f"SHAP Explanation: High-Risk Bridge {test_df.loc[high_risk_idx, 'bridge_id']}"
    )
    plt.tight_layout()
    ...
    plt.title(
        f"SHAP Explanation: Low-Risk Bridge {test_df.loc[low_risk_idx, 'bridge_id']}"
    )
    plt.tight_layout()
    ...
    for col in ["bridge_age", "DECK_COND_058"]:
        if col in X_test_df.columns:
            ...
            plt.savefig(
                os.path.join(FIGURES_DIR, f"shap_dependence_{safe_col}.png"), dpi=150
            )
            plt.close()
