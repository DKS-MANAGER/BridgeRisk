...
def main():
    ...
    for year in YEARS:
        ...
        for col in CONDITION_COLS:
            if col in df.columns:
                ...
                report_lines.append(f"  {col}: {sorted(unique_vals)[:20]} (total: {len(unique_vals)})")
            else:
                report_lines.append(f"  {col}: NOT FOUND")
        
        report_lines.append("\nDuplicate records check:")
        if all(c in df.columns for c in ID_COLS):
            df["bridge_id"] = df[ID_COLS[0]].astype(str).str.strip() + "_" + df[ID_COLS[1]].astype(str).str.strip()
            dup_count = df["bridge_id"].duplicated().sum()
            report_lines.append(f"  Duplicate bridge IDs: {dup_count}")
        else:
            report_lines.append(f"  ID columns not found, cannot check duplicates")
    ...
    if all(c in dfs[2023].columns for c in ID_COLS) and \
       all(c in dfs[2024].columns for c in ID_COLS) and \
       all(c in dfs[2025].columns for c in ID_COLS):
        dfs[2023]["bridge_id"] = dfs[2023][ID_COLS[0]].astype(str).str.strip() + "_" + dfs[2023][ID_COLS[1]].astype(str).str.strip()
        dfs[2024]["bridge_id"] = dfs[2024][ID_COLS[0]].astype(str).str.strip() + "_" + dfs[2024][ID_COLS[1]].astype(str).str.strip()
        dfs[2025]["bridge_id"] = dfs[2025][ID_COLS[0]].astype(str).str.strip() + "_" + dfs[2025][ID_COLS[1]].astype(str).str.strip()
        ...
        report_lines.append("Cannot perform matching - ID columns not found in all years.")
    
    report_text = "\n".join(report_lines)
    with open(os.path.join(REPORTS_DIR, "data_inspection.txt"), "w", encoding="utf-8") as f:
        f.write(report_text)
    print(report_text)
    ...
    if matching_counts:
        ...
        matching_df.to_csv(os.path.join(REPORTS_DIR, "matching_counts.csv"), index=False)
